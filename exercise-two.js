...
function problemaA () {
  ...
  */
  ...
  // Versión Promesas
  const pEstrofa1 = promisifiedReadFile('poema-dos/estrofa-01.txt')
   .then(blue)

  const pEstrofa2 = promisifiedReadFile('poema-dos/estrofa-02.txt')
   .then(blue)

  Promise.all([pEstrofa1, pEstrofa2])
...
function problemaD () {
  ...
  */
  ...
  // Versión Promesas
  Promise.map(filenames, promisifiedReadFile) //-->[pr1,pr2,pr3]
  //estrofas es un array de promesas
   .then(estrofas => estrofas.forEach(blue))
   .catch(magenta)
   .then(() => console.log('done'));
}
